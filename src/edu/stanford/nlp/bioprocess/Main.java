...
public class Main implements Runnable {
    ...
    List<BioDatum> SRLPredicted;

    public List<BioDatum> getSRLPredictions() {
        assert(mode.equals("srl"));
        run();
        return SRLPredicted;
    }
	...
	public void run() {
		...
		if(mode.equals("entity")) {
            ...
            SRLPredicted = runSRLPrediction(folders);
		}
	...
	public List<BioDatum> runSRLPrediction(HashMap<String, String> folders) {
		int NumCrossValidation = 5;
		boolean small = false;
		...
		if (small) {
			...
			Triple<Double, Double, Double> triple = Scorer.scoreSRL(dataset.examples("sample"), result);
			LogInfo.logs("Precision : " + triple.first);
			LogInfo.logs("Recall    : " + triple.second);
			LogInfo.logs("F1 score  : " + triple.third);
