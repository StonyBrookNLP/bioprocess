...
public class EntityFeatureFactory extends FeatureExtractor {
	...
	//					}
						...
						BioDatum newDatum = new BioDatum(sentence, Utils.getText(node), type, node, event.getTreeNode(), Utils.getArgumentMentionRelation(event, node).toString(), ex.id);
    ...
    public List<BioDatum> setFeaturesTest(CoreMap sentence, Set<Tree> predictedEvents, String exampleID) {
		...
		for(Tree eventNode: predictedEvents) {
			for(Tree node: sentence.get(TreeCoreAnnotations.TreeAnnotation.class)) {
				...
				BioDatum newDatum = new BioDatum(sentence, Utils.getText(node), type, node, eventNode, Utils.getArgumentMentionRelation(sentence, eventNode, node).toString(), exampleID);
